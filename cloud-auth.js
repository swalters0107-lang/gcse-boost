/* LevelUp10 V0.23.1 — Parent Accounts
...
(() => {
  ...
  async function signUp(email,password,displayName,requestedRole="student"){message("Creating account…");const {data,error}=await client.auth.signUp({email,password,options:{data:{display_name:displayName||"Learner",avatar:window.gcseBoostPendingAvatar||"🎓",requested_role:requestedRole}}});if(error)return message(error.message,"error");if(!data.session)return message("Account created. Check your email to confirm it, then sign in.","good");if(requestedRole!=="student"){const roleResult=await client.rpc("levelup10_set_own_role",{new_role:requestedRole});if(roleResult.error)console.warn("Role setup pending database migration",roleResult.error)}loadedUserId=null;await loadCloudIdentity(true);renderCloudAccount()}
  ...
  function summariseState(state){
    ...
    const mastery=s.mastery||s.topicMastery||s.masteryByTopic||{};
    const vals=Object.values(mastery).map(Number).filter(Number.isFinite);
    const masteryAvg=vals.length?Math.round(vals.reduce((a,b)=>a+b,0)/vals.length):null;
    return {level:Number(s.level||s.currentLevel)||null,masteryAvg};
  }
  async function renderParentDashboard(){
    ...
    host.innerHTML=rows.map(r=>{const x=summariseState(r.state);return `<div class="cloud-student-card"><div><b>${esc(r.display_name||"Student")}</b><span>${esc(r.avatar||"🎓")}</span></div><div class="cloud-stat-grid"><div><strong>${Number(r.xp)||0}</strong><small>XP</small></div><div><strong>${Number(r.streak)||0}</strong><small>Streak</small></div><div><strong>${x.level||"—"}</strong><small>Level</small></div><div><strong>${x.masteryAvg==null?"—":x.masteryAvg+"%"}</strong><small>Mastery</small></div></div><p class="cloud-small">Read-only progress · Last updated ${r.updated_at?new Date(r.updated_at).toLocaleString():"—"}</p></div>`}).join("");
  }

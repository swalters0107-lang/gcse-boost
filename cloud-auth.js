/* LevelUp10 V0.23.3.2 — Baseline Handoff Fix
...
(() => {
  ...
  async function showTeacherClass(index){
    ...
    const lessonCurriculum={"Maths":{"All Maths":["Adaptive mixed Maths"],"Number":["Number skills","Fractions, decimals & percentages","Powers, roots & standard form"],"Algebra":["Solving linear equations","Expressions & formulae","Sequences","Graphs","Inequalities","Quadratics"],"Ratio, Proportion & Rates of Change":["Ratio","Proportion","Rates of change"],"Geometry & Measures":["Angles","Area, perimeter & volume","Transformations","Pythagoras & trigonometry"],"Probability":["Probability"],"Statistics":["Statistics"]},"Science":{"All Science":["Adaptive mixed Science"],"Biology":["Biology"],"Chemistry":["Chemistry"],"Physics":["Physics"]},"English":{"All English":["Adaptive mixed English"],"Paper 1 Reading":["Paper 1 Reading"],"Paper 1 Writing":["Paper 1 Writing"],"Paper 2 Reading":["Paper 2 Reading"],"Paper 2 Writing":["Paper 2 Writing"],"Language Skills":["Language Skills"]}};
    const classSubject=(cls.subject||"Maths").trim(),curriculum=lessonCurriculum[classSubject]||{["All "+classSubject]:["Adaptive mixed "+classSubject]},topicKeys=Object.keys(curriculum),savedTopic=currentLesson?String(currentLesson.lesson_topic||""):"",initialTopic=topicKeys.includes(savedTopic)?savedTopic:topicKeys[0];
    const topicOptions=topicKeys.map(t=>`<option value="${esc(t)}"${t===initialTopic?" selected":""}>${esc(t)}</option>`).join(""),focusOptions=(curriculum[initialTopic]||[]).map(f=>`<option value="${esc(f)}">${esc(f)}</option>`).join("");

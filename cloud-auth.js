/* GCSE Boost V0.11B.3 — Cloud Load Repair
...
(() => {
  ...
  let client=null,currentUser=null,currentProfile=null,loadedUserId=null,cloudLoadState="idle",cloudLoadError="",newLearnerInitialised=false;
  ...
  function ensureUi(){
    ...
    const localBtn=document.querySelector(".account-home-btn"); if(!localBtn)return;
    const btn=document.createElement("button"); btn.id="cloudAccountBtn";btn.className="cloud-account-btn";btn.type="button";
    btn.innerHTML='☁️ <b>Cloud account</b><span id="cloudAccountStatus">Not signed in</span>';
    btn.addEventListener("click",showCloudAccount); localBtn.insertAdjacentElement("afterend",btn);
  }
  ...
  async function loadCloudProgress(force=false){
    ...
    try{
      ...
      if(isFreshCloudLearner && typeof window.gcseBoostConfigureCloudLearner==="function"){
        window.gcseBoostConfigureCloudLearner({userId:currentUser.id,name:currentProfile?.display_name||currentUser.user_metadata?.display_name||"Learner",avatar:currentUser.user_metadata?.avatar||"🎓"});
  ...
  async function loadCloudIdentity(forceProgress=false){
    ...
    if(!currentUser){currentProfile=null;loadedUserId=null;setStatus("Not signed in",false);return}
    await fetchProfile();setStatus(currentProfile?.display_name||currentUser.email||"Connected",true);await loadCloudProgress(forceProgress);
  }
  ...
  async function showCloudAccount(){closeCloudAccount();const o=document.createElement("div");o.id="cloudAccountOverlay";o.className="cloud-overlay";o.innerHTML='<div class="cloud-panel" role="dialog" aria-modal="true" aria-label="Cloud account"><div class="cloud-head"><div><b>☁️ Cloud Account</b><small>V0.11B.4</small></div><button id="cloudClose" type="button" aria-label="Close">×</button></div><div id="cloudAccountBody"><p>Checking connection…</p></div></div>';document.body.appendChild(o);document.getElementById("cloudClose").onclick=closeCloudAccount;o.addEventListener("click",e=>{if(e.target===o)closeCloudAccount()});await loadCloudIdentity();renderCloudAccount()}

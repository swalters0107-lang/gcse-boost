...
(() => {
  ...
  async function signUp(email,password,displayName){message("Creating account…");const {data,error}=await client.auth.signUp({email,password,options:{data:{display_name:displayName||"Learner"}}});if(error)return message(error.message,"error");if(!data.session)return message("Account created. Check your email to confirm it, then sign in.","good");loadedUserId=null;await loadCloudIdentity(true);renderCloudAccount()}
  ...
  async function showCloudAccount(){closeCloudAccount();const o=document.createElement("div");o.id="cloudAccountOverlay";o.className="cloud-overlay";o.innerHTML='<div class="cloud-panel" role="dialog" aria-modal="true" aria-label="Cloud account"><div class="cloud-head"><div><b>☁️ Cloud Account</b><small>V0.11B.3</small></div><button id="cloudClose" type="button" aria-label="Close">×</button></div><div id="cloudAccountBody"><p>Checking connection…</p></div></div>';document.body.appendChild(o);document.getElementById("cloudClose").onclick=closeCloudAccount;o.addEventListener("click",e=>{if(e.target===o)closeCloudAccount()});await loadCloudIdentity();renderCloudAccount()}
